...
import parse from 'html-react-parser';
import PricingAccordion from './PricingAccordion';
import ZaochkaFlexboxComponent from './ZaochkaFlexboxComponent';
...
export default function DirectionsClient({ directions, directionContent, zaochkaDirections, zaochkaDirectionContent, acf, tarifImages }) {
    ...
    // Добавляем состояние для тултипов заочки
    ...
    // Тексты для тултипов
    const grantTooltipText = "Грант — это финансовая поддержка, предоставляемая университетом для покрытия части или полной стоимости обучения, основанная на академических достижениях или других критериях.";
    const stipendiyaTooltipText = "Стипендия — это регулярные выплаты студентам за успехи в учебе, научной деятельности или другие достижения, предоставляемые университетом или другими организациями.";

    // Преобразуем данные заочки в формат для карточек
    const zaochnoePrograms = zaochkaDirections && zaochkaDirectionContent ?
        Object.keys(zaochkaDirections).map(directionKey => {
            const direction = zaochkaDirections[directionKey];
            const content = zaochkaDirectionContent[directionKey];

            return {
                title: directionKey,
                price: content?.stats?.find(stat => stat.label.includes('семестр'))?.value || 'Цена не указана',
                description: content?.description?.replace(/<[^>]*>/g, '').substring(0, 200) + '...' || 'Описание отсутствует',
                duration: content?.duration || '4 года',
                granty: direction?.granty || false,
                stipendiya: direction?.stipendiya || false,
                firstShift: direction?.firstShift || false,
                secondShift: direction?.secondShift || false,
                bgGradient: 'bg-gradient-to-br from-purple-600 to-pink-600',
                bgGradient_2: 'bg-gradient-to-br from-[#8f3178] to-[#6b0e55]',
    ...
    return (
        <div className="bg-gray-50 min-h-screen font-sans">
            ...
            <div className="max-w-screen-xl mx-auto px-6 py-4">
                ...
                {/* Tabs - стилизованные */}
                <div className="mb-10">
                    <div className="w-[100%] md:w-auto inline-flex flex-col md:flex-row gap-[10px] p-2 bg-gray-100 rounded-xl">
                        {tabs.map((tab) => (
                            <button
                                key={tab.id}
                                onClick={() => setActiveTab(tab.id)}
                ...
                {/* Условный рендеринг: для заочки - flexbox карточки, для остальных - обычный layout */}
                {activeTab === 'zaochka' ? (
                    // Flexbox для заочного направления

                    // В компоненте
                    <ZaochkaFlexboxComponent />
                ) : (
                    // Content Layout для медицинских и бизнес направлений
                    <div className="md:flex gap-6">
                        {/* Direction Sidebar - стилизованный */}
                        <div className="md:w-1/4 md:mb-[0] mb-6">
                            <div className="bg-white rounded-lg shadow-md overflow-hidden">
                        ...
                        <div className="md:w-3/4">
                            <div className="bg-white rounded-lg shadow-md overflow-hidden">
                                ...
                                >
                                    ...
                                    <h2
                                        className="text-3xl font-bold mb-3 relative z-10 text-white"
                                    >
                                        {getCurrentDirectionTitle()}
                                    </h2>
                                    <p
