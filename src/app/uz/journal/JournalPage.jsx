...
export default function JournalPage({ pageData, brandColors }) {
    ...
    const journals = acf?.stati_v_zhurnale?.map((journal) => ({
        ...
        number: journal.nazvanie_uz.match(/№(\d+)/)?.[1] || 'Unknown',
        year: journal.god,
        downloadUrl: journal.fajl_zhurnala.url,
        coverUrl: journal.oblozhka.url,
        name: journal.nazvanie_uz // Добавляем название журнала
    })) || [];

    const editorialBoard = acf?.testovyj_blok_info_uz || {};
    const rules = acf?.testovyj_blok_info_uz?.spisok_pravil || {};
    const rulesFileUrl = acf?.testovyj_blok_info_uz?.fajl_pravil_oformleniya?.url || '';
