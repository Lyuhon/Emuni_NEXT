...
//     useEffect(() => {
//         // Состояние загрузки в начале
//         setIsLoading(true);

//         // Устанавливаем таймер для имитации минимального времени загрузки
//         const timer = setTimeout(() => {
//             setIsLoading(false);

//             // Плавный скролл к контейнеру после загрузки, если изменился параметр page
...
//         // Создаем функцию отслеживания изменений URL
//         const handleRouteChange = () => {
//             setIsLoading(true);
//             // Максимальное время загрузки (если сервер долго отвечает)
...
//         // Находим все ссылки навигации блога
...
export default function PostsWithLoader({ children }) {
    ...
    useEffect(() => {
        ...
        const handleLinkClick = (e) => {
            const href = e.currentTarget.getAttribute("href");
            if (href && href.startsWith("/uz/blog")) {
                setIsLoading(true);
            }
        };
        ...
        // Добавляем обработчики
        window.addEventListener("popstate", handleRouteChange);
        const blogLinks = document.querySelectorAll('a[href^="/uz/blog"]');
        blogLinks.forEach((link) => {
            link.addEventListener("click", handleLinkClick);
        });
