...
//             `${wpBaseUrl}/custom/v1/post-by-slug/${slug}?lang=uz`,
...
//         if (postData.code === "no_post" || postData.language !== "uz") {
//             console.error(`No post found for slug: ${slug} or language is not uz: ${postData.language}`);
//             return { post: null, relatedPosts: [], sidebarContent: {} };
//         }
...
//                 `${wpBaseUrl}/wp/v2/posts?_embed&per_page=3&categories=${formattedPost.categoryId}&exclude=${post.id}&lang=uz`,
//                 { next: { revalidate: 86400 } }
//             );
...
//             quickLinks: [
//                 // { title: "Dars jadvali", href: "/schedule" },
//                 // { title: "Hujjat topshirish", href: "/admissions" },
//                 { title: "Yo‘nalishlar", href: "/uz/napravleniya" },
//                 { title: "Ilmiy jurnal", href: "/uz/journal" },
//                 { title: "Ko‘p so‘raladigan savollar", href: "/uz/faq" },
//             ],
//             contacts: [
//                 { label: "Telefon", value: "+998(78) 147-00-07" },
//                 { label: "Email", value: "info@emuni.uz" },
//                 { label: "Manzil", value: "Muqimiy ko‘chasi 7/1, Toshkent, O‘zbekiston" },
//             ],
//         };
...
//                                 <h3 className="text-lg font-semibold text-gray-800 mb-3">Ushbu maqolani baham ko'ring:</h3>
...
// \src\app\uz\blog\[slug]\page.jsx

import Link from "next/link";
import style from "./post.css";

// SEO метаданные (динамические для каждого поста)
export async function generateMetadata({ params }) {
    ...
    if (!post) {
        return {
            title: "Sahifa topilmadi | EMU University",
            description: "So'ralgan maqola mavjud emas yoki foydalanib bo'lmaydi.",
        };
    }

    return {
        title: `${post.title} | EMU University blogi`,
        description: post.description,
        keywords: `${post.category}, EMU University, tibbiyot universiteti, universitet blogi, Toshkentda ta'lim`,
        openGraph: {
            title: post.title,
            description: post.description,
            images: [post.image],
...
// Функция для получения метаданных поста для SEO
...
async function fetchPostData(slug) {
    ...
    try {
        ...
        const formattedPost = {
            ...
            title: decodeHTMLEntities(post.title),
            image: post.featured_image,
            category: post.category.name || "Bez kategorii", // Используем название категории
            categoryId: post.category.slug ? await getCategoryId(post.category.slug) : 0,
            categorySlug: post.category.slug || "uncategorized", // Слаг для ссылки
            content: formatContent(post.content),
            date: post.date
            ...
            if (relatedPostsRes.ok) {
                ...
                relatedPosts = relatedPostsData.map((related) => ({
                    ...
                    image:
                        ...
                        "https://next.emu.web-perfomance.uz/wp-content/uploads/2025/01/placeholder-image-1024x1024.png",
                    slug: related.slug,
                    category: decodeHTMLEntities(related._embedded?.["wp:term"]?.[0]?.[0]?.name) || "Bez kategorii",
                }));
            }
        }
        ...
        const sidebarContent = {
            quickLinks: [
                // { title: "Dars jadvali", href: "/uz/schedule" },
                // { title: "Hujjat topshirish", href: "/uz/admissions" },
                { title: "Yo'nalishlar", href: "/uz/napravleniya" },
                { title: "Ilmiy jurnal", href: "/uz/journal" },
                { title: "Ko'p so'raladigan savollar", href: "/uz/faq" },
            ],
            ...
            <div className="min-h-screen bg-gradient-to-br from-gray-50 to-gray-100 flex justify-center items-center">
                <div className="bg-white rounded-xl shadow-md p-8 max-w-md text-center animate-fadeIn">
                    <div className="relative mb-6">
                        ...
                        <h1 className="text-2xl font-bold text-[#6b0e55]">Xatolik</h1>
                    </div>
                    <p className="text-gray-600 mb-6">Maqola identifikatori ko'rsatilmagan.</p>
                    <Link
                        href="/uz/blog"
        ...
        return (
            <div className="min-h-screen bg-gradient-to-br from-gray-50 to-gray-100 flex justify-center items-center relative overflow-hidden">
                ...
                <div className="bg-white rounded-xl shadow-md p-8 max-w-md text-center animate-fadeIn relative z-10">
                    <div className="relative mb-6">
                        <div className="absolute -top-4 -left-4 w-12 h-12 bg-[#6b0e55]/10 rounded-lg transform rotate-12 animate-spin-slow" />
                        <h1 className="text-2xl font-bold text-[#6b0e55]">Maqola topilmadi</h1>
                    </div>
                    <p className="text-gray-600 mb-6">So'ralgan maqola mavjud emas yoki o'zbek tilida mavjud emas.</p>
                    <Link
                        href="/uz/blog"
                        className="inline-block bg-[#6b0e55] text-white px-6 py-3 rounded-lg hover:bg-[#8f3178] transition-colors duration-300 shadow-md hover:shadow-lg"
                    >
        ...
        <div className="min-h-screen bg-gradient-to-br from-gray-50 to-gray-100">
            ...
            <script type="application/ld+json" dangerouslySetInnerHTML={{
                __html: JSON.stringify({
                    ...
                    "mainEntityOfPage": {
                        "@type": "WebPage",
                        "@id": `https://emuni.uz/uz/blog/${post.slug}`
                    }
                })
            }} />
            ...
            <div className="bg-white relative overflow-hidden">
                ...
                <div className="max-w-screen-xl mx-auto px-4 md:px-8 py-12 md:py-24">
                    <div className="relative z-10 w-full text-center">
                        ...
                        <div className="flex flex-wrap justify-center items-center gap-3 mt-4">
                            <Link
                                href={`/uz/blog?category=${post.categorySlug}`}
                        ...
                        <div className="p-6 md:p-8">
                            ...
                            <div className="mt-8 pt-6 border-t border-gray-100">
                                ...
                                <div className="flex space-x-3">
                                    ...
                                    <a href={`https://t.me/share/url?url=${encodeURIComponent(`https://next.emu.web-perfomance.uz/uz/blog/${post.slug}`)}&text=${encodeURIComponent(post.title)}`}
                                        target="_blank"
                                        rel="noopener noreferrer"
                                        className="w-10 h-10 flex items-center justify-center rounded-full bg-[#0088cc] text-white hover:opacity-90 transition-opacity"
                                    ...
                                    <a href={`https://vk.com/share.php?url=${encodeURIComponent(`https://next.emu.web-perfomance.uz/uz/blog/${post.slug}`)}&title=${encodeURIComponent(post.title)}`}
                                        target="_blank"
                                        rel="noopener noreferrer"
                                        className="w-10 h-10 flex items-center justify-center rounded-full bg-[#45668e] text-white hover:opacity-90 transition-opacity"
                                        aria-label="VKontakte orqali ulashish"
                                    >
                                        <svg className="w-5 h-5" viewBox="0 0 24 24" fill="currentColor">
                                    ...
                                    <a href={`https://api.whatsapp.com/send?text=${encodeURIComponent(`${post.title} - https://next.emu.web-perfomance.uz/uz/blog/${post.slug}`)}`}
                                        target="_blank"
                                        rel="noopener noreferrer"
                                        className="w-10 h-10 flex items-center justify-center rounded-full bg-[#25D366] text-white hover:opacity-90 transition-opacity"
