...
//     let postsUrl = `${wpBaseUrl}/posts?lang=uz&per_page=${postsPerPage}&page=${page}`;

//     try {
//         // Fetch categories
//         const categoriesRes = await fetch(`${wpBaseUrl}/categories?lang=uz`, {
...
//         const categories = [
//             { id: "all", slug: "all", name: "Barcha postlar" },
...
// // Вспомогательная функция для оптимизации пагинации
...
//         if (page > 3) items.push(page - 1);
//         items.push(page);
//         if (page < totalPages - 2) items.push(page + 1);
//     }

//     if (page >= totalPages - 1 && page > 3) {
//         if (totalPages > 3) items.push(totalPages - 2);
//     }
...
//                             <div className="absolute -left-4 -top-4 w-12 h-12 bg-[#5f1464]/10 rounded-lg transform rotate-12 [animation:spin_4s_linear_infinite]" />
//                             <h1 className="text-4xl md:text-6xl font-bold text-gray-900 mb-6">
//                                 Blog
//                                 <br />
//                                 <span className="text-[#5f1464] relative">
//                                     EMU Universiteti
//                                     <div className="absolute -bottom-2 left-0 w-full h-1 bg-[#5f1464]/20 transform skew-x-[12deg]" />
//                                 </span>
//                             </h1>
//                         </div>
//                         <p className="text-lg md:text-xl text-gray-600 max-w-3xl mx-auto">
//                             Talabalar va abituriyentlar uchun yangiliklar, voqealar va foydali maqolalar
//                         </p>
...
//                                 <summary className="text-left px-6 py-4 flex justify-between items-center bg-[#5f1464] text-white rounded-t-xl cursor-pointer">
...
//                                         <li key={cat.id}>
//                                             <Link
//                                                 href={`/uz/blog?category=${cat.slug}`}
...
//                             <Link
//                                 href={`/uz/blog?category=${category}&page=${Math.max(page - 1, 1)}`}
//                                 className={`p-1 sm:p-2 rounded-full bg-[#5f1464] text-white ${page === 1 ? "opacity-50 cursor-not-allowed" : "hover:bg-[#7a407f]"
//                                     } transition-colors`}
//                             >
//                                 <svg
...
//                                         <Link
//                                             key={item}
//                                             href={`/uz/blog?category=${category}&page=${item}`}
...
//                             </div>
//                             <Link
//                                 href={`/uz/blog?category=${category}&page=${Math.min(page + 1, totalPages)}`}
//                                 className={`p-1 sm:p-2 rounded-full bg-[#5f1464] text-white ${page === totalPages ? "opacity-50 cursor-not-allowed" : "hover:bg-[#7a407f]"
//                                     } transition-colors`}
//                             >
//                                 <svg
//                                     className="w-4 h-4 sm:w-5 sm:h-5"
...
export const metadata = {
    ...
    description: "EMU University rasmiy blogi: talabalar va abituriyentlar uchun foydali maqolalar, dolzarb yangiliklar, tadbirlar e’lonlari va muvaffaqiyat hikoyalari.",
    keywords: "EMU University blogi, tibbiyot universiteti yangiliklari, abituriyentlar uchun maqolalar, EMUdagi tadbirlar, tibbiy ta’lim",
    openGraph: {
        title: "Blog, yangiliklar, maqolalar va tadbirlar - EMU University",
...
async function fetchBlogData(category = "all", page = 1) {
    ...
    try {
        ...
        const posts = postsData.posts.map((post) => ({
            ...
            image: post.featured_image,
            category: post.category.slug || "uncategorized",
            categoryName: decodeHTMLEntities(post.category.name) || "Bez kategorii",
            description: decodeHTMLEntities(post.excerpt).slice(0, 150).trim() + "...",
            slug: post.slug,
            language: post.language || "uz",
            date: post.date
        ...
        console.error("Error fetching blog data:", error);
        return { posts: [], categories: [{ id: "all", slug: "all", name: "Barcha postlar" }], totalPages: 1 };
    }
}

// Обновленная функция пагинации без многоточия для 5 или менее страниц
function getPaginationItems(page, totalPages) {
        ...
        <div className="min-h-screen bg-gradient-to-br from-gray-50 to-gray-100">
            ...
            <script type="application/ld+json" dangerouslySetInnerHTML={{
                __html: JSON.stringify({
                    ...
                    "@type": "Blog",
                    "headline": "EMU Universiteti blogi",
                    "description": "EMU Universitetining rasmiy blogi - talabalar va abituriyentlar uchun yangiliklar, maqolalar va tadbirlar",
                    "url": "https://emuni.uz/uz/blog",
                    "publisher": {
                        "@type": "Organization",
                        "name": "EMU University",
                        "logo": {
                    ...
                    "blogPost": posts.slice(0, 3).map(post => ({
                        ...
                        "url": `https://emuni.uz/uz/blog/${post.slug}`
                    }))
                })
            }} />
                    ...
                    <PostsWithLoader>
                        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 md:gap-8">
                            {posts.length > 0 ? (
                                ...
                                <div className="col-span-full text-center py-12">
                                    <h3 className="text-xl text-gray-600">Maqolalar topilmadi</h3>
                                    <p className="mt-2 text-gray-500">Boshqa kategoriyani tanlashga harakat qiling</p>
                                </div>
                            )}
                        </div>
                    </PostsWithLoader>
