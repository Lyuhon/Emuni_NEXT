...
export async function generateMetadata({ params }) {
    ...
    if (!post) {
        return {
            title: "Страница не найдена | EMU University",
            description: "Запрашиваемая статья не существует или недоступна.",
        };
    }

    return {
        title: `${post.title} - Блог EMU University`,
        description: post.description,
        keywords: `${post.category}, EMU University, блог университета, образование в Ташкенте`,
        openGraph: {
            title: post.title,
            description: post.description,
            images: [post.image],
...
async function getPostMetadata(slug) {
    try {
        ...
        return {
            ...
            category: postData.category.name || "Блог",
        };
    } catch (error) {
        return null;
    }
        ...
        return (
            <div className="min-h-screen bg-gradient-to-br from-gray-50 to-gray-100 flex justify-center items-center relative overflow-hidden">
                ...
                <div className="bg-white rounded-xl shadow-md p-8 max-w-md text-center animate-fadeIn relative z-10">
                    <div className="relative mb-6">
                        <div className="absolute -top-4 -left-4 w-12 h-12 bg-[#6b0e55]/10 rounded-lg transform rotate-12 animate-spin-slow" />
                        <h1 className="text-2xl font-bold text-[#6b0e55]">Статья не найдена</h1>
                    </div>
                    <p className="text-gray-600 mb-6">Запрашиваемая статья не существует или недоступна на русском языке.</p>
                    <Link
                        href="/blog"
        ...
        <div className="min-h-screen bg-gradient-to-br from-gray-50 to-gray-100">
            ...
            <script type="application/ld+json" dangerouslySetInnerHTML={{
                __html: JSON.stringify({
                    ...
                    "mainEntityOfPage": {
                        "@type": "WebPage",
                        "@id": `https://emuni.uz/blog/${post.slug}`
                    }
                })
            }} />
                        ...
                        <div className="p-6 md:p-8">
                            ...
                            <div className="mt-8 pt-6 border-t border-gray-100">
                                ...
                                <div className="flex space-x-3">
                                    ...
                                    <a href={`https://t.me/share/url?url=${encodeURIComponent(`https://next.emu.web-perfomance.uz/blog/${post.slug}`)}&text=${encodeURIComponent(post.title)}`}
                                        target="_blank"
                                        rel="noopener noreferrer"
                                        className="w-10 h-10 flex items-center justify-center rounded-full bg-[#0088cc] text-white hover:opacity-90 transition-opacity"
                                        aria-label="Поделиться в Telegram"
                                    ...
                                    <a href={`https://vk.com/share.php?url=${encodeURIComponent(`https://next.emu.web-perfomance.uz/blog/${post.slug}`)}&title=${encodeURIComponent(post.title)}`}
                                        target="_blank"
                                        rel="noopener noreferrer"
                                        className="w-10 h-10 flex items-center justify-center rounded-full bg-[#45668e] text-white hover:opacity-90 transition-opacity"
                                        aria-label="Поделиться ВКонтакте"
                                    ...
                                    <a href={`https://api.whatsapp.com/send?text=${encodeURIComponent(`${post.title} - https://next.emu.web-perfomance.uz/blog/${post.slug}`)}`}
                                        target="_blank"
                                        rel="noopener noreferrer"
                                        className="w-10 h-10 flex items-center justify-center rounded-full bg-[#25D366] text-white hover:opacity-90 transition-opacity"
                                        aria-label="Поделиться в WhatsApp"
