...
import '../clinic-base/clinic-base.css';

// SEO metadata
export const metadata = {
    title: "Clinical Training Bases - EMU University",
    description: "Partner clinics and clinical training bases of EMU University for practical education of medical specialty students",
    keywords: "clinical bases, partner clinics, medical education, student practice, EMU University, medical faculty",
    openGraph: {
        title: "Clinical Training Bases of EMU University",
        description: "Partner medical institutions for practical training of EMU University students",
...
const structuredData = {
    ...
    "name": "EMU University",
    "description": "Medical partner clinics of EMU University",
    "url": "https://emu.uz/clinical-bases",
    "department": {
        "@type": "MedicalOrganization",
        "name": "Medical Faculty of EMU University",
        "medicalSpecialty": "Training in medical specialties"
    }
};
...
export default async function ClinicalBasesPage() {
    const response = await fetch('https://next.emu.web-perfomance.uz/wp-json/acf/v3/pages/83611', {
        next: { revalidate: 86400 }, // Update every 24 hours
    });
    const data = await response.json();
    const title = data.acf.zagolovok_straniczy_ang || data.acf.zagolovok_straniczy_ru;
    const description = data.acf.kratkoe_opisanie_ang || data.acf.kratkoe_opisanie_ru;
    const clinics = data.acf.kliniki;

    const clinicalBases = clinics.map(clinic => ({
        name: clinic.nazvanie_ang || clinic.nazvanie_ru,
        location: clinic.adres_ang || clinic.adres_ru,
        image: clinic.fotka_kliniki.url,
        mapLink: clinic.ssylka_na_yandeks_karty_kliniki, // Adding link to Yandex Maps
    }));
