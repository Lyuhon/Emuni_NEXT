...
export default function RegistrationFormEng() {
    // Form states
    ...
    // Set current URL in "Source" field
    useEffect(() => {
        if (fields.length > 0) {
    ...
    // Define field visibility conditions
    const directionField = fields.find((f) => f.label === 'Choose a direction');
    ...
    if (isLoading) {
        return (
            <div className="flex flex-col items-center justify-center py-8">
                ...
                <p className="text-[#6b0e55] text-lg">Loading form fields...</p>
            </div>
        );
    }

    if (fields.length === 0) {
        return <p className="text-gray-500 text-center">Loading fields...</p>;
    }
                        ...
                        <div key={field.id} className="space-y-2">
                            ...
                            {field.type === 'select' && (
                                <select
                                    ...
                                    className={`w-full border border-[#f9eef5] rounded-lg p-3 focus:ring-1 focus:ring-[#3b82f6] focus:border-[#3b82f6] focus-visible:outline-none hover:border-[#8f3178] transition-all duration-300 transition-colors appearance-none bg-[url('/images/icons/arrow.svg')] bg-no-repeat bg-[right_1rem_center] bg-[length:16px_16px] ${isLanguageFieldDisabled ? 'bg-gray-100 cursor-not-allowed opacity-75' : ''}`}
                                >
                                    <option value="">{field.placeholder || '- Choose -'}</option>

                                    {/* For "Select language" field when INTERNATIONAL is selected, add "English" option */}
                                    {field.label === 'Select language' && showInternationalFaculty && (
                                        <option value="English">English</option>
                                    )}

                                    {/* Special logic for "Choose a direction" field */}
                                    {field.label === 'Choose a direction' ? (
                                        ...
                                        /* For all other selects use standard logic */
