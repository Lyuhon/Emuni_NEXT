...
const ArticleSubmissionForm = ({ isOpen, onClose }) => {
    ...
    const handleFileChange = (e) => {
        ...
        if (file) {
            ...
            if (!file.name.endsWith('.docx')) {
                setErrors({
                    ...errors,
                    file: 'Please upload a .docx format file'
                });
                return;
            }
            ...
            if (file.size > 10 * 1024 * 1024) {
                setErrors({
                    ...errors,
                    file: 'File size should not exceed 10 MB'
                });
                return;
            }
    ...
    const validateForm = () => {
        ...
        if (!formData.authorName.trim()) {
            newErrors.authorName = 'Please enter the author\'s full name';
        }

        if (!formData.title.trim()) {
            newErrors.title = 'Please enter the article title';
        }

        if (!formData.workplace.trim()) {
            newErrors.workplace = 'Please specify the workplace';
        }

        if (!formData.file) {
            newErrors.file = 'Please upload the article file';
        }
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            if (response.ok) {
                setMessage('Article successfully submitted! We will contact you shortly.');
                setFormData({
                    authorName: '',
                    coauthors: '',
                    title: '',
                ...
                setMessage(`Error during submission: ${data.message || 'Something went wrong'}`);
            }
        } catch (error) {
            console.error('Error submitting form:', error);
            setMessage('Error during submission. Please try again later.');
        } finally {
                ...
                >
                    ...
                    >
                        <div className="flex justify-between items-center mb-6">
                            <h2 className="text-2xl font-bold text-gray-800">Submit Article</h2>
                            <button
                                onClick={onClose}
                        ...
                        <form onSubmit={handleSubmit} className="space-y-4">
                            <div className="space-y-2">
                                <label className="block text-sm font-semibold text-gray-700">
                                    Author's Full Name <span className="text-red-500">*</span>
                                </label>
                                <input
                                    type="text"
                                    name="authorName"
                            ...
                            <div className="space-y-2">
                                ...
                                <input
                                    ...
                                    onChange={handleChange}
                                    placeholder="Enter co-authors' full names (if any)"
                            ...
                            <div className="space-y-2">
                                <label className="block text-sm font-semibold text-gray-700">
                                    Article File (.docx, up to 10 MB) <span className="text-red-500">*</span>
                                </label>
