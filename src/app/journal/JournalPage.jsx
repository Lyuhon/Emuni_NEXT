...
export default function JournalPage({ pageData }) {
    ...
    const title = acf?.zagolovok_straniczy_ru || 'Журнал';
    const description = acf?.kratkoe_opisanie_ru || '';
    ...
    const journals = acf?.stati_v_zhurnale?.map((journal) => ({
        ...
        number: journal.nazvanie_ru.match(/№(\d+)/)?.[1] || 'Unknown',
        year: journal.god,
        downloadUrl: journal.fajl_zhurnala.url,
        coverUrl: journal.oblozhka.url,
        name: journal.nazvanie_ru // Добавляем название журнала
    })) || [];

    const editorialBoard = acf?.testovyj_blok_info_ru || {};
    const rules = acf?.testovyj_blok_info_ru?.spisok_pravil || {};
    const rulesFileUrl = acf?.testovyj_blok_info_ru?.fajl_pravil_oformleniya?.url || '';
                        ...
                        <div className="relative z-10 w-full md:w-1/2 mb-8 md:mb-0">
                            ...
                            <div className="mt-8 flex flex-wrap gap-4">
                                {/* <button
                                    ...
                                    className="transition px-8 py-3 bg-[#5f1464] text-white rounded-full hover:bg-opacity-90 transition-colors transform hover:-translate-y-1 duration-200"
                                >
                                    Отправить статью
                                </button>
                ...
                <div className="max-w-screen-xl mx-auto px-4 md:px-8">
                    ...
                    <div className="flex flex-col md:flex-row justify-center space-y-4 md:space-y-0 md:space-x-6">
                        ...
                        >
                            ...
                            <span className="text-sm md:text-base">Просмотреть правила</span>
                        </a>
                    </div>
                </div>
            </div>
