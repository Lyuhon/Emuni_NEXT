...
export default function UzPopup() {
    ...
    // Добавьте этот маппинг для INTERNATIONAL факультетов после mappingBus:
    const mappingInternational = {
    ...
    // /==============/
    // Добавьте этот useEffect для очистки полей при смене направления:
    useEffect(() => {
        if (fields.length === 0) return;
        ...
        if (currentDirection) {
            // Получаем поля, которые нужно очистить при смене направления
            const medFacultyField = fields.find(f => f.label === 'Fakul\'tetni tanlang (MEDICAL SCHOOL)');
            const busFacultyField = fields.find(f => f.label === 'Fakul\'tetni tanlang (BUSINESS AND SOCIAL SCHOOL)');
            const intFacultyField = fields.find(f => f.label === 'Select faculty (INTERNATIONAL)');
            const medTimeField = fields.find(f => f.label === 'Ta\'lim shakli (MEDICAL SCHOOL)');
            const busTimeField = fields.find(f => f.label === 'Ta\'lim shakli (BUSINESS AND SOCIAL SCHOOL)');
            const directionBitrixField = fields.find(f => f.label === 'BITRIX - Направления');
            const timeBitrixField = fields.find(f => f.label === 'BITRIX - Форма обучения');
            ...
            // Очищаем поля в зависимости от выбранного направления
            if (currentDirection === 'MEDICAL SCHOOL') {
                // Очищаем поля бизнеса и международного направления
                if (busFacultyField && formData[busFacultyField.id]) {
                    updatedFormData[busFacultyField.id] = '';
                    needClear = true;
                }
                if (intFacultyField && formData[intFacultyField.id]) {
                    updatedFormData[intFacultyField.id] = '';
                    needClear = true;
                }
                if (busTimeField && formData[busTimeField.id]) {
                    updatedFormData[busTimeField.id] = '';
                    needClear = true;
                }
            } else if (currentDirection === 'BUSINESS AND SOCIAL SCHOOL') {
                // Очищаем поля медицины и международного направления
                if (medFacultyField && formData[medFacultyField.id]) {
                    updatedFormData[medFacultyField.id] = '';
                    needClear = true;
                }
                if (intFacultyField && formData[intFacultyField.id]) {
                    updatedFormData[intFacultyField.id] = '';
                ...
                if (medTimeField && formData[medTimeField.id]) {
                    updatedFormData[medTimeField.id] = '';
                    needClear = true;
                }
            } else if (currentDirection === 'INTERNATIONAL') {
                // Очищаем поля медицины и бизнеса
                if (medFacultyField && formData[medFacultyField.id]) {
                    updatedFormData[medFacultyField.id] = '';
                    needClear = true;
                }
            ...
            // Если очистили какие-то поля, также очищаем BITRIX поля
            if (needClear) {
                if (directionBitrixField) {
                    updatedFormData[directionBitrixField.id] = '';
                }
                if (timeBitrixField) {
                    updatedFormData[timeBitrixField.id] = '';
                }
                setFormData(updatedFormData);
            }
        }
    }, [formData[fields.find(f => f.label === "Yo'nalishni tanlang")?.id], fields.length]);

    // И обновите useEffect для синхронизации полей (замените существующий):
    useEffect(() => {
        if (fields.length === 0) return;
        ...
        // Логика для INTERNATIONAL направления
        if (directionField && languageField && currentDirection === 'INTERNATIONAL') {
            if (formData[languageField.id] !== 'Ingliz') {
        ...
        if (currentDirection === 'INTERNATIONAL' && intFacultyField && directionBitrixField && formData[intFacultyField.id]) {
            const selectedValue = formData[intFacultyField.id];
            const mappedValue = mappingInternational[selectedValue];
            if (mappedValue && formData[directionBitrixField.id] !== mappedValue) {
                if (!updatedFormData) updatedFormData = { ...formData };
                updatedFormData[directionBitrixField.id] = mappedValue;
                needUpdate = true;
        ...
        formData[fields.find(f => f.label === "Yo'nalishni tanlang")?.id],
        formData[fields.find(f => f.label === 'Fakul\'tetni tanlang (MEDICAL SCHOOL)')?.id],
        formData[fields.find(f => f.label === 'Fakul\'tetni tanlang (BUSINESS AND SOCIAL SCHOOL)')?.id],
        formData[fields.find(f => f.label === 'Select faculty (INTERNATIONAL)')?.id],
        formData[fields.find(f => f.label === 'Ta\'lim shakli (MEDICAL SCHOOL)')?.id],
        formData[fields.find(f => f.label === 'Ta\'lim shakli (BUSINESS AND SOCIAL SCHOOL)')?.id],
        formData[fields.find(f => f.label === 'Telefon raqamingiz')?.id],
        formData[fields.find(f => f.label === 'Q\'oshimcha telefon raqam')?.id],
        fields.length
    ]);
    // /==============/
    ...
    const handleSubmit = async (e) => {
        ...
        const directionField = fields.find((f) => f.label === "Yo'nalishni tanlang");
        const directionValue = formData[directionField?.id] || '';
        const submitFormId = directionValue === 'BUSINESS AND SOCIAL SCHOOL' ? 572 : formId;
